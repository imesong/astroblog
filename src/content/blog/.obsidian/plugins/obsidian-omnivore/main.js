  ...
  "node_modules/@0no-co/graphql.web/dist/graphql.web.js"(exports) {
    ...
    function error(e2) {
      return new GraphQLError(
        `Syntax Error: Unexpected token at ${r} in ${e2}`
      );
    }
    function advance(i2) {
      i2.lastIndex = r;
    ...
    var g = {
      OperationDefinition(e2) {
        if (
          e2.operation === "query" &&
          !e2.name &&
          !hasItems(e2.variableDefinitions) &&
          !hasItems(e2.directives)
        ) {
        ...
        if (hasItems(e2.variableDefinitions)) {
          ...
          r2 +=
            "(" +
            e2.variableDefinitions.map(g.VariableDefinition).join(", ") +
            ")";
        }
      ...
      Field(e2) {
        ...
        return e2.selectionSet
          ? r2 + " " + g.SelectionSet(e2.selectionSet)
          : r2;
      },
      StringValue: e2 =>
        e2.block ? printBlockString(e2.value) : printString(e2.value),
      ...
      Document: e2 =>
        hasItems(e2.definitions) ? e2.definitions.map(print).join("\n\n") : "",
      SelectionSet: e2 =>
        "{\n  " +
    ...
    exports.valueFromTypeNode = function valueFromTypeNode(e2, r2, i2) {
      if (e2.kind === "Variable") {
        ...
        switch (r2.name.value) {
          ...
          case "String":
          case "Bool":
            return r2.name.value + "Value" === e2.kind
              ? valueFromASTUntyped(e2, i2)
              : void 0;
          default:
    ...
    exports.visit = function visit(e2, r2) {
      ...
      try {
        var t2 = (function traverse(e3, t3, a2) {
          var o2 = false;
          var l3 =
            (r2[e3.kind] && r2[e3.kind].enter) || r2[e3.kind] || r2.enter;
...
var require_wonka = __commonJS({
  "node_modules/wonka/dist/wonka.js"(exports) {
    ...
    function fromAsyncIterable(e2) {
      return r2 => {
        var t2 =
          (e2[asyncIteratorSymbol()] && e2[asyncIteratorSymbol()]()) || e2;
        var i = false;
        var s2 = false;
        var a = false;
        var f;
    ...
    exports.fromObservable = function fromObservable(e2) {
      return r2 => {
        var t2 = (
          e2[observableSymbol()] ? e2[observableSymbol()]() : e2
        ).subscribe({
          next(e3) {
            r2(push2(e3));
          },
...
var require_urql_core_chunk = __commonJS({
  "node_modules/@urql/core/dist/urql-core-chunk.js"(exports) {
    ...
    var CombinedError = class extends Error {
      constructor(input) {
        var normalizedGraphQLErrors = (input.graphQLErrors || []).map(
          rehydrateGraphQlError
        );
        var message = generateErrorMessage(
          input.networkError,
    ...
    var REPLACE_CHAR_RE = /(?:#[^\n\r]+)?(?:[\r\n]+|$)/g;
    var replaceOutsideStrings = (str, idx) =>
      idx % 2 === 0 ? str.replace(REPLACE_CHAR_RE, "\n") : str;
    var sanitizeDocument = node =>
...
# ${operationName}`,
    ...
    var mergeResultPatch = (prevResult, nextResult, response, pending) => {
      ...
      if (incremental) {
        ...
        withData.data =
          (nextResult.payload || nextResult).data || prevResult.data;
        errors =
          nextResult.errors ||
    ...
    var splitOutSearchParams = url => {
      ...
      return start2 > -1
        ? [url.slice(0, start2), new URLSearchParams(url.slice(start2 + 1))]
    ...
    function makeFetchSource(operation, url, fetchOptions) {
      ...
      return wonka.onEnd(() => {
        ...
        wonka.filter(result => !!result)(
          wonka.fromAsyncIterable(fetchOperation(operation, url, fetchOptions))
        )
      );
    }
...
var require_urql_core = __commonJS({
  "node_modules/@urql/core/dist/urql-core.js"(exports) {
    ...
    var collectTypenames = response => [
      ...collectTypes(response, /* @__PURE__ */ new Set()),
    ];
    var formatNode = node => {
      ...
      if ("selectionSet" in node) {
        ...
        if (node.selectionSet) {
          for (var selection of node.selectionSet.selections || []) {
            hasTypename =
              hasTypename ||
              (selection.kind === graphql_web.Kind.FIELD &&
    ...
    function withPromise(_source$) {
      ...
      source$.toPromise = () =>
        wonka.toPromise(
          wonka.take(1)(
            wonka.filter(result => !result.stale && !result.hasNext)(source$)
          )
        );
    ...
    var shouldSkip = ({ kind }) => kind !== "mutation" && kind !== "query";
    ...
    var cacheExchange = ({ forward, client, dispatchDebug }) => {
      ...
      return ops$ => {
        var cachedOps$ = wonka.map(operation => {
            ...
            ? dispatchDebug({
                ...
                ...(cachedResult
                  ...
                  : {
                      type: "cacheMiss",
                      message:
                        "The result could not be retrieved from the cache",
                    }),
          ...
          var result =
            cachedResult ||
            fetchSource.makeResult(operation, {
              data: null,
            });
          result = {
            ...result,
            operation: true
              ? addMetadata(operation, {
          ...
          forward(
            wonka.filter(
              ...
              wonka.map(op =>
                ...
                wonka.merge([
                  wonka.map(mapTypeNames)(
                    wonka.filter(
                      op => !shouldSkip(op) && !isOperationCached(op)
                    )(ops$)
                  ),
                  wonka.filter(op => shouldSkip(op))(ops$),
                ])
              )
            )
          )
    ...
    var ssrExchange = (params = {}) => {
        ...
        ops$ => {
          var isClient =
            params && typeof params.isClient === "boolean"
              ? !!params.isClient
              : !client.suspense;
          var forwardedOps$ = forward(
            wonka.map(mapTypeNames)(
              wonka.filter(
                operation =>
                  operation.kind === "teardown" ||
                  !data2[operation.key] ||
                  !!data2[operation.key].hasNext ||
                  operation.context.requestPolicy === "network-only"
              )(ops$)
            )
          );
          var cachedOps$ = wonka.map(op => {
            ...
            var cachedResult = deserializeResult(
              op,
              serialized,
              includeExtensions
            );
      ...
      ssr.extractData = () => {
        ...
        for (var _key2 in data2)
          if (data2[_key2] != null) result[_key2] = data2[_key2];
        return result;
      };
      ...
      ({ client, forward }) => {
        var createSubscriptionSource = operation => {
          var observableish = forwardSubscription(
            fetchSource.makeFetchBody(operation),
            operation
          );
          return wonka.make(observer => {
            ...
            function nextResult(value) {
              observer.next(
                (result = result
                  ? fetchSource.mergeResultPatch(result, value)
                  : fetchSource.makeResult(operation, value))
              );
            }
            Promise.resolve().then(() => {
              ...
              sub = observableish.subscribe({
                ...
                error(error) {
                  if (Array.isArray(error)) {
                    ...
                    observer.next(
                      fetchSource.makeErrorResult(operation, error)
                    );
                  }
                  observer.complete();
                },
        ...
        return ops$ => {
          var subscriptionResults$ = wonka.mergeMap(operation => {
            var { key } = operation;
            var teardown$ = wonka.filter(
              op => op.kind === "teardown" && op.key === key
            )(ops$);
            return wonka.takeUntil(teardown$)(
    ...
    var dedupExchange =
      ...
      ops$ =>
        forward(ops$);
    var fetchExchange = ({ forward, dispatchDebug }) => {
      return ops$ => {
        var fetchResults$ = wonka.mergeMap(operation => {
          ...
          true
            ...
            : void 0;
          var source = wonka.takeUntil(
            wonka.filter(
              op => op.kind === "teardown" && op.key === operation.key
            )(ops$)
          )(fetchSource.makeFetchSource(operation, url, fetchOptions));
          if (true) {
    ...
    var mapExchange = ({ onOperation, onResult, onError }) => {
      return ({ forward }) =>
        ops$ => {
          return wonka.mergeMap(result => {
            ...
            return "then" in newResult
              ? wonka.fromPromise(newResult)
              : wonka.fromValue(newResult);
          })(
            forward(
              wonka.mergeMap(operation => {
                ...
                return "then" in newOperation
                  ? wonka.fromPromise(newOperation)
                  : wonka.fromValue(newOperation);
              })(ops$)
            )
          );
        };
    ...
    var Client = function Client2(opts) {
            ...
            return result.stale || result.hasNext
              ...
              : wonka.merge([
                  ...
                  wonka.map(() => {
                    ...
                    wonka.take(1)(
                      wonka.filter(op => op.key === operation.key)(
                        operations.source
                      )
                    )
                  ),
      ...
      var instance =
        this instanceof Client2 ? this : Object.create(Client2.prototype);
      var client = Object.assign(instance, {
        ...
        createRequestOperation(kind, request, opts2) {
          ...
          if (
            kind !== "teardown" &&
            (requestOperationType = fetchSource.getOperationType(
              request.query
            )) !== kind
          ) {
            throw new Error(
          ...
          return makeOperation(kind, request, {
            ...
            suspense:
              opts2.suspense || (opts2.suspense !== false && client.suspense),
          });
        },
          ...
          return withPromise(
            wonka.lazy(() => {
              ...
              if (!source2) {
                active.set(
                  operation.key,
                  (source2 = makeResultSource(operation))
                );
              }
              ...
              if (
                ...
                return wonka.switchMap(wonka.fromValue)(
                  wonka.merge([
                    source2,
                    wonka.filter(
                      replay2 => replay2 === replays.get(operation.key)
                    )(wonka.fromValue(replay)),
                  ])
                );
              } else {
                return source2;
        ...
        executeSubscription(query, opts2) {
          var operation = client.createRequestOperation(
            "subscription",
            query,
            opts2
          );
        ...
        query(query, variables, context) {
          return client.executeQuery(
            fetchSource.createRequest(query, variables),
            context
          );
        },
      ...
      if (true) {
        var { next, source } = wonka.makeSubject();
        client.subscribeToDebugTarget = onEvent =>
          wonka.subscribe(onEvent)(source);
        dispatchDebug = next;
      }
      var composedExchange = composeExchanges(opts.exchanges);
      var results$ = wonka.share(
        composedExchange({
          client,
          dispatchDebug,
...
var require_errors = __commonJS({
  "node_modules/@omnivore-app/api/dist/src/errors.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.isOmnivoreError =
      exports.buildOmnivoreError =
      exports.OmnivoreErrorCode =
        void 0;
    ...
    function initGraphQLTada() {
      function graphql(e2, n2) {
        ...
        if (a[0].kind === r.Kind.FRAGMENT_DEFINITION && a[0].directives) {
          a[0].directives = a[0].directives.filter(
            r2 => r2.name.value !== "_unmask"
          );
        }
        return {
...
var require_graphql = __commonJS({
  "node_modules/@omnivore-app/api/dist/src/graphql.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.saveByURLMutation =
      exports.DeleteMutation =
      exports.UpdatesSinceQuery =
      exports.SearchQuery =
        void 0;
    ...
    var Omnivore2 = class {
      constructor(clientOptions) {
        ...
        __publicField(this, "items", {
          search: async params => {
            const { data: data2, error } = await this.client
              .query(graphql_1.SearchQuery, {
                ...params,
          ...
          updates: async params => {
            const { data: data2, error } = await this.client
              .query(graphql_1.UpdatesSinceQuery, params)
              .toPromise();
          ...
          delete: async params => {
            const { data: data2, error } = await this.client
              .mutation(graphql_1.DeleteMutation, {
            ...
            if (
              ...
              const err = (0, errors_1.buildOmnivoreError)(
                deleteArticle,
                error
              );
          ...
          saveByUrl: async params => {
            const { data: data2, error } = await this.client
              .mutation(graphql_1.saveByURLMutation, {
                input: {
                  ...params,
...
var require_src = __commonJS({
  "node_modules/@omnivore-app/api/dist/src/index.js"(exports) {
    ...
    Object.defineProperty(exports, "Omnivore", {
      enumerable: true,
      get: function () {
    ...
    var errors_1 = require_errors();
    Object.defineProperty(exports, "OmnivoreErrorCode", {
      enumerable: true,
      get: function () {
        return errors_1.OmnivoreErrorCode;
      },
    });
    Object.defineProperty(exports, "isOmnivoreError", {
      enumerable: true,
      get: function () {
    ...
    (function () {
      ...
      var runInContext = function runInContext2(context) {
        ...
        function hashHas(key) {
          var data2 = this.__data__;
          return nativeCreate
            ? data2[key] !== undefined2
            : hasOwnProperty2.call(data2, key);
        }
        ...
        function baseFill(array, value, start2, end2) {
          ...
          end2 =
            end2 === undefined2 || end2 > length ? length : toInteger(end2);
          if (end2 < 0) {
            end2 += length;
          }
        ...
        function baseInRange(number, start2, end2) {
          return (
            number >= nativeMin(start2, end2) &&
            number < nativeMax(start2, end2)
          );
        }
        ...
        function castSlice(array, start2, end2) {
          ...
          return !start2 && end2 >= length
            ? array
            : baseSlice(array, start2, end2);
        }
        var clearTimeout2 =
          ctxClearTimeout ||
          function (id) {
          ...
          return flatRest(function (funcs) {
            ...
            while (++index < length) {
              ...
              var funcName = getFuncName(func),
                data2 = funcName == "wrapper" ? getData(func) : undefined2;
              if (
                data2 &&
                isLaziable(data2[0]) &&
                data2[1] ==
                ...
                wrapper = wrapper[getFuncName(data2[0])].apply(
                  wrapper,
                  data2[3]
                );
              } else {
        ...
        function createRange(fromRight) {
          return function (start2, end2, step) {
            ...
            step =
              step === undefined2 ? (start2 < end2 ? 1 : -1) : toFinite(step);
        ...
        function mergeData(data2, source) {
          ...
          if (srcBitmask & WRAP_ARY_FLAG) {
            data2[8] =
              data2[8] == null ? source[8] : nativeMin(data2[8], source[8]);
          }
          if (data2[9] == null) {
            data2[9] = source[9];
          }
        ...
        var stringToPath = memoizeCapped(function (string) {
          ...
          string.replace(
            rePropName,
            function (match3, number, quote, subString) {
              result2.push(
                quote ? subString.replace(reEscapeChar, "$1") : number || match3
              );
            }
          );
          return result2;
        ...
        function slice(array, start2, end2) {
          ...
          if (
            end2 &&
            typeof end2 != "number" &&
            isIterateeCall(array, start2, end2)
          ) {
            start2 = 0;
            end2 = length;
          } else {
        ...
        function sampleSize(collection, n2, guard) {
          if (
            guard ? isIterateeCall(collection, n2, guard) : n2 === undefined2
          ) {
            n2 = 1;
          } else {
            n2 = toInteger(n2);
        ...
        function trimEnd(string, chars, guard) {
          ...
          var strSymbols = stringToArray(string),
            end2 = charsEndIndex(strSymbols, stringToArray(chars)) + 1;
...
var require_diff_match_patch = __commonJS({
  "node_modules/diff-match-patch/index.js"(exports, module2) {
    ...
    diff_match_patch2.prototype.diff_main = function (
      text1,
      text2,
      opt_checklines,
      opt_deadline
    ...
    diff_match_patch2.prototype.diff_compute_ = function (
      text1,
      text2,
      checklines,
      deadline
    ...
    diff_match_patch2.prototype.diff_lineMode_ = function (
      text1,
      text2,
      deadline
    ) {
    ...
    diff_match_patch2.prototype.diff_bisect_ = function (
      text1,
      text2,
      deadline
    ) {
    ...
    diff_match_patch2.prototype.diff_bisectSplit_ = function (
      text1,
      text2,
      x,
      y,
    ...
    diff_match_patch2.prototype.diff_charsToLines_ = function (
      diffs,
      lineArray
    ) {
    ...
    diff_match_patch2.prototype.diff_cleanupSemanticLossless = function (
      diffs
    ) {
      function diff_cleanupSemanticScore_(one, two) {
        ...
        var nonAlphaNumeric1 = char1.match(
          diff_match_patch2.nonAlphaNumericRegex_
        );
        var nonAlphaNumeric2 = char2.match(
          diff_match_patch2.nonAlphaNumericRegex_
        );
        var whitespace1 =
          nonAlphaNumeric1 && char1.match(diff_match_patch2.whitespaceRegex_);
        var whitespace2 =
          nonAlphaNumeric2 && char2.match(diff_match_patch2.whitespaceRegex_);
        var lineBreak1 =
          whitespace1 && char1.match(diff_match_patch2.linebreakRegex_);
        var lineBreak2 =
          whitespace2 && char2.match(diff_match_patch2.linebreakRegex_);
        var blankLine1 =
          lineBreak1 && one.match(diff_match_patch2.blanklineEndRegex_);
        var blankLine2 =
    ...
    diff_match_patch2.prototype.diff_fromDelta = function (text1, delta) {
      ...
      for (var x = 0; x < tokens.length; x++) {
        ...
        switch (tokens[x].charAt(0)) {
          case "+":
            try {
              diffs[diffsLength++] = new diff_match_patch2.Diff(
                DIFF_INSERT,
                decodeURI(param)
              );
            } catch (ex) {
    ...
    diff_match_patch2.prototype.patch_deepCopy = function (patches) {
      ...
      for (var x = 0; x < patches.length; x++) {
        ...
        for (var y = 0; y < patch.diffs.length; y++) {
          patchCopy.diffs[y] = new diff_match_patch2.Diff(
            patch.diffs[y][0],
            patch.diffs[y][1]
          );
        }
      ...
      for (var x = 0; x < patches.length; x++) {
        ...
        while (bigpatch.diffs.length !== 0) {
          ...
          if (precontext !== "") {
            ...
            patch.diffs.push(
              new diff_match_patch2.Diff(DIFF_EQUAL, precontext)
            );
          }
          while (
            ...
            if (diff_type === DIFF_INSERT) {
              ...
              empty = false;
              patch.diffs.push(
                new diff_match_patch2.Diff(diff_type, diff_text)
              );
          ...
          if (postcontext !== "") {
            ...
            if (
              ...
              patch.diffs.push(
                new diff_match_patch2.Diff(DIFF_EQUAL, postcontext)
              );
            }
          }
          if (!empty) {
    ...
    module2.exports = function (string, skips) {
      ...
      return replacements.reduce(function (string2, replacement) {
        ...
        return name && skips.indexOf(name) !== -1
          ? string2
          : string2.replace(replacement[0], replacement[1]);
      }, string);
    };
  },
});
...
function formatOffset(offset3, format) {
  const hours = Math.trunc(Math.abs(offset3 / 60)),
    minutes = Math.trunc(Math.abs(offset3 % 60)),
    sign = offset3 >= 0 ? "+" : "-";
  switch (format) {
    case "short":
...
function extractISOOffset(match3, cursor) {
  const local = !match3[cursor] && !match3[cursor + 1],
    fullOffset = signedOffset(match3[cursor + 1], match3[cursor + 2]),
...
var Interval = class {
  ...
  static fromDateTimes(start2, end2) {
    const builtStart = friendlyDateTime(start2),
      builtEnd = friendlyDateTime(end2);
  ...
  static after(start2, duration) {
    const dur = Duration.fromDurationLike(duration),
      dt = friendlyDateTime(start2);
    return Interval.fromDateTimes(dt, dt.plus(dur));
  }
  static before(end2, duration) {
    const dur = Duration.fromDurationLike(duration),
      dt = friendlyDateTime(end2);
  ...
  count(unit = "milliseconds", opts) {
    ...
    return (
      Math.floor(end2.diff(start2, unit).get(unit)) +
      (end2.valueOf() !== this.end.valueOf())
    );
  }
  hasSame(unit) {
    return this.isValid
...
function diffRelative(start2, end2, opts) {
  const round2 = isUndefined(opts.round) ? true : opts.round,
    format = (c, unit) => {
  ...
  while (!scanner.eos()) {
    ...
    if (type === "#" || type === "^") {
      ...
      if (openSection[1] !== value)
        throw new Error(
          'Unclosed section "' + openSection[1] + '" at ' + start2
        );
...
Scanner.prototype.scan = function scan(re) {
  ...
  if (!match3 || match3.index !== 0) return "";
  var string = match3[0];
...
Writer.prototype.escapedValue = function escapedValue(token, context, config) {
  ...
  var value = context.lookup(token[1]);
  if (value != null)
    return typeof value === "number" && escape2 === mustache.escape
      ? String(value)
      : escape2(value);
};
...
var isEmoji$1 = function isEmoji(text, i) {
  ...
  if (text[i + 1]) {
    ...
    if (
      isHighSurrogate(_code) ||
      isLowSurrogate(_code) ||
      isVariationSelector(_code)
    ) {
      return true;
    }
  }
...
var renderItemContent = async (
  ...
  if (frontMatterTemplate) {
    const frontMatterTemplateRendered = mustache_default.render(
      frontMatterTemplate,
      articleView
    );
    try {
      const frontMatterParsed = (0, import_obsidian3.parseYaml)(
...
var FolderSuggest = class extends TextInputSuggest {
  getSuggestions(inputStr) {
    ...
    abstractFiles.forEach(folder => {
      if (
        folder instanceof import_obsidian5.TFolder &&
...
var OmnivoreSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian6.Setting(renderHighlightConfigContainer)
      ...
      .addToggle(toggle =>
        toggle
          .setValue(
            this.plugin.settings.highlightManagerId == "hltr" /* HIGHLIGHTR */
          )
          .onChange(async value => {
    ...
    this.displayBlock(
      ...
      this.plugin.settings.highlightManagerId == "omni" /* OMNIVORE */
    );
    const highlighterSetting = new import_obsidian6.Setting(
      omnivoreHighlightConfigContainer
    );
    const colorPickers = {};
    highlighterSetting
    ...
    Object.entries(colorMap).forEach(([colorName, hexCode]) => {
      ...
      const colorSetting = new import_obsidian6.Setting(settingItem)
        .setName(colorName)
        .setDesc(hexCode);
    ...
    new import_obsidian6.Setting(advancedSettings)
...
var OmnivorePlugin = class extends import_obsidian7.Plugin {
  ...
  async downloadFileAsAttachment(item) {
    ...
    const folderName = (0, import_obsidian7.normalizePath)(
      render3(
        item,
    ...
    const fileName = (0, import_obsidian7.normalizePath)(
      `${folderName}/${item.id}.pdf`
    );
    ...
    try {
      ...
      for (let after = 0; ; after += size) {
        const [items, hasNextPage] = await getItems(
          ...
          after,
          size,
          parseDateTime(syncAt).toISO() || void 0,
          customQuery,
          includeContent,
          "highlightedMarkdown"
        );
        for (const item of items) {
          const folderName = replaceIllegalCharsFolder(
            (0, import_obsidian7.normalizePath)(
              render3(item, folder, this.settings.folderDateFormat)
            )
          );
...
${newContentWithoutFrontMatter}`
            ...
            await this.app.fileManager.processFrontMatter(
              ...
              async frontMatter => {
                ...
                if (id && id !== item.id) {
                  ...
                  const newNormalizedPath = (0, import_obsidian7.normalizePath)(
                    newPageName
                  );
  ...
  async deleteCurrentItem(file) {
    ...
    if (!itemId) {
      new import_obsidian7.Notice(
        "Failed to delete article: article id not found"
      );
    }
    try {
